...
import { visaImmigrationAPI } from '../utils/api';
import { useFetch, useFormSubmit } from '../hooks/useAPI';

export default function VisaImmigrationAdmin() {
  const [activeTab, setActiveTab] = useState('applications');
  const [searchTerm, setSearchTerm] = useState('');
  const [formData, setFormData] = useState({ name: '', email: '', visaType: '', country: '' });

  const { data, loading, error, refetch } = useFetch(
    () => visaImmigrationAPI.getApplications(1, 50),
    []
  ) as any;

  const { handleSubmit, loading: submitLoading, error: submitError } = useFormSubmit(
    async (formData) => {
      await visaImmigrationAPI.createApplication(formData);
    }
  );
  ...
  const stats = [
    { label: 'Total Applications', value: data?.total || '267', color: 'bg-purple-500' },
    { label: 'Approved', value: data?.stats?.approved || '198', color: 'bg-green-500' },
    { label: 'In Process', value: data?.stats?.inProcess || '52', color: 'bg-orange-500' },
    { label: 'Rejected', value: data?.stats?.rejected || '17', color: 'bg-red-500' },
  ];
  ...
  return (
    <div className="bg-white min-h-screen p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-[#054374] mb-8">Visa & Immigration Admin</h1>
              ...
              <table className="w-full">
                <thead className="bg-gray-100 border-b">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-sm font-semibold">Visa Type</th>
                ...
                <tbody>
                  ...
                  filteredApplications.map((app: any) => (
                    <tr key={app.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">{app.visaType}</td>
                      ...
                      <td className="px-6 py-4 flex gap-2">
                        ...
                        <button
                          onClick={async () => {
                            await visaImmigrationAPI.deleteApplication(app.id);
                            refetch();
                          }}
        ...
        {activeTab === 'analytics' && (
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-white p-6 rounded-lg shadow">
              <h3 className="text-lg font-bold mb-4">Applications by Country</h3>
              <div className="space-y-3">
                {[
                  { country: 'USA', count: 89, percentage: 33 },
                  { country: 'Canada', count: 67, percentage: 25 },
                  { country: 'UK', count: 56, percentage: 21 },
                  { country: 'Australia', count: 55, percentage: 21 },
                ].map(item => (
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              <h3 className="text-lg font-bold mb-4">Visa Type Distribution</h3>
              <div className="space-y-3">
                {[
                  { type: 'Student Visa', percentage: 60 },
                  { type: 'Work Visa', percentage: 25 },
                  { type: 'Family Visa', percentage: 10 },
        ...
        {activeTab === 'settings' && (
          <div className="bg-white p-6 rounded-lg shadow max-w-2xl">
            <h3 className="text-lg font-bold mb-6">Visa Settings</h3>
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium mb-2">Average Processing Time (days)</label>
                <input type="number" defaultValue={60} className="w-full border rounded px-3 py-2" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-medium mb-2">Default Visa Officer</label>
              ...
              <button className="bg-purple-600 text-white px-6 py-2 rounded hover:bg-purple-700 transition">Save Settings</button>
            </div>
          </div>
        )}
      </div>
